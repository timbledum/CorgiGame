...
class Corgi:
    ...
    def update_corgi_location(self):
        ...
        if pyxel.btn(pyxel.KEY_SPACE):
            ...
            if self.velocity >= -10:
                ...
                if self.velocity < 0:
                    neg = -1
                new_location = old_location._replace(y = old_location.y-((self.velocity**2)*0.1*neg))
                self.velocity -=1
                self.corgi_run_right(new_location)
            else :                 
                self.velocity =10
    ...
    def corgi_run_right(self, new_location):
        ...
        if self.run_animation_right<=25:
            pyxel.blt(x=new_location.x, y= self.corgi_location.y, img = 0, u = 0, v=16* (self.run_animation_right // 5), w=11, h=12, colkey=0)#+16h
